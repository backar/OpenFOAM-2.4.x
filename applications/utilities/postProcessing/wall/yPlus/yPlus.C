...
Description
    ...
    Updated 2014 by Jon-Anders Bäckar to acknowledge the -region option and to
    report correct yPlus also for zeroGradient boundary condition assuming
    no-slip, i.e. U=(0 0 0) at the wall.
    Use the -noSlip option e.g. when using wall sources in combination with a
...
int main(int argc, char *argv[])
{
    ...
    (
        "noSlip",
        "assuming no-slip for slip and zeroGradient boundaries"
    );
    ...
    (
        "uTauInfo",
        "Show uTau info, used in channelFlow post-process"
    );
    ...
    const bool noSlip = args.optionFound("noSlip");
...
#           include "createPhi.H"
        ...
        forAll(patches, patchi)
        {
            ...
            {
                vectorField dUdy = U.boundaryField()[patchi].snGrad();
                bool isZeroGrad = isA<zeroGradientFvPatchVectorField>(
                    U.boundaryField()[patchi]);
                bool isSlip = isA<slipFvPatchField<vector> >(
                    U.boundaryField()[patchi]);
                if (noSlip && (isZeroGrad || isSlip))
                {
                    forAll(dUdy, facei)
                    {
                        dUdy[facei] = U.boundaryField()[patchi][facei]/
                            d[patchi][facei];
                    }
                    Info << dUdy << endl;
                }
