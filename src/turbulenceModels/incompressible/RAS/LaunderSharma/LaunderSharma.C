...
defineTypeNameAndDebug(LaunderSharma, 0);
addToRunTimeSelectionTable(RASModel, LaunderSharma, dictionary);

// * * * * * * * * * * * * Private Member Functions  * * * * * * * * * * * * //

tmp<volScalarField> LaunderSharma::fMu() const
{
    return exp(-3.4/sqr(scalar(1) + sqr(k_)/(nu()*epsilonTilda_)/50.0));
}


tmp<volScalarField> LaunderSharma::f2() const
{
    return
        scalar(1)
...
tmp<volSymmTensorField> LaunderSharma::R() const
{
...
tmp<volSymmTensorField> LaunderSharma::devReff() const
{
    return tmp<volSymmTensorField>
    (
        new volSymmTensorField
...
tmp<fvVectorMatrix> LaunderSharma::divDevReff(volVectorField& U) const
{
    return
    (
...
tmp<fvVectorMatrix> LaunderSharma::divDevRhoReff
(
